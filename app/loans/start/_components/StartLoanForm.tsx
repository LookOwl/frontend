...
import { LoanError, startLoan } from "@/lib/api/loans";
import { getCurrentUser, getSession } from "@/lib/auth/session";

export function StartLoanForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      await startLoan({ copyCode: code }, session.accessToken);
      setIsSuccess(true);
      setCopyCode("");
    } catch (err) {
  ...
  if (!isLibrarian) {
    return (
      ...
      >
        Solo los bibliotecarios pueden registrar préstamos.
      </p>
    );
  }
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-xs text-zinc-500 dark:text-zinc-400">
          La solicitud de préstamo de esta copia ya debe tener el ejemplar
          asignado.
        </p>
      </div>
      ...
      {isSuccess ? (
        ...
        >
          Préstamo registrado. El ejemplar quedó entregado al lector.
        </p>
      ) : null}
